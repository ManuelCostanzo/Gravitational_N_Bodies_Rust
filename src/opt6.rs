...
fn run(galaxy: &mut Galaxy) {
    ...
    (0..D).for_each(|_| {
        let forces = poses
            ...
            .zip(masses.par_chunks_exact(BLOCK_SIZE))
            .map(|(possesj, massesj)| {
                zip(&possesj[..], &massesj[..]).map(|(pj, massj)| {
                    let force = zip(&poses[..], &masses[..]).fold(
                        Force::new(),
                        |force_acc, (pi, massi)| {
                            let dx = pi.x - pj.x;
                            let dy = pi.y - pj.y;
                            ...
                            let d32 = a / dsquared.get().sqrt().powi(3);
                            let f = G * *massj * *massi;
                            Force::new_with(
        ...
        bodies
            ...
            .zip(forces)
            .for_each(|(bodies, forces)| {
                zip(&mut bodies[..], forces).for_each(|(body, force)| {
...
fn main() {
    ...
    let name: String = std::env::current_exe()
        ...
        .to_owned()
        .into();
    println!("{} {} {}, auto {}", name, N, BLOCK_SIZE, time);

    let mut f = std::fs::OpenOptions::new()
        .append(true)
        .open("times.csv")
        .unwrap();
    write!(&mut f, "{}, {}, {}, {}\n", name, N, BLOCK_SIZE, time).unwrap();
}
